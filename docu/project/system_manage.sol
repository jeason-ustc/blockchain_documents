...
contract system_manage {
  ...
  function register(address contract_address,uint user_deposit_amount) public returns (bool success) {
    ...
    if(msg.sender != depository[contract_address].owner_addr){
        return false;
    }
  ...
  function rent(address contract_address,uint rent_value) public returns (bool success) {
     if(depository[contract_address].rent != 0) {
         return false;
     }
     if(rent_value != depository[contract_address].rent_value){
         return false;
     }
     registrantsPaid[msg.sender] = deposit_quota;
	 if(!depository[contract_address].owner_addr.send(depository[contract_address].rent_value)){
	     throw;
	 }
	 rent_info_map[msg.sender]=depository[contract_address].owner_addr;
	 user_list.push(msg.sender);
	 user_amount++;
	 depository[contract_address].rent=1;
	 return true;
  }
  ...
  function build_map(address contract_add,address owner_add,uint rent_flag,uint value) public returns (bool success) {
